...
def _resolve_config_path(agent_name: str, config_path: str | None, base_dir: Path) -> Path:
...
async def claude_agent_sdk_monitor(
    ...
    try:
        async with MCPServerManager(
            agent_name, base_dir=base_dir, config_path=resolved_config
        ) as manager:
...
def main() -> None:
    ...
    asyncio.run(
        claude_agent_sdk_monitor(args.agent_name, config_path=args.config, model=args.model)
    )
