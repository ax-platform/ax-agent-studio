...
def test_message_context_extraction():
    ...
    for test in test_cases:
        ...
        if sender == test["sender"]:
            print("    PASS: Sender correctly extracted")
        else:
            print("    FAIL: Sender mismatch!")
            assert False
        ...
        if f"@{test['sender']}" in message_with_context:
            ...
            print("    FAIL: Sender not found in LLM context!")
            assert False
...
def test_conversation_reply_format():
    ...
    for scenario in scenarios:
        ...
        if f"@{scenario['to']}" in scenario["wrong_reply"]:
            print("     WARNING: Wrong reply contains self-mention!")
